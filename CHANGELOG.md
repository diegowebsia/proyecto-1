...
- **Ayuda contextual en todos los módulos**: cada panel (`ReviewCard`, `BillingPanel`,
  ...
  (clic, hover, foco y Escape) con explicación en lenguaje no técnico y salto directo a la
  sección correspondiente del Centro de ayuda, reescrito sin tecnicismos ni HTTP codes.
- **Integraciones 100 % asistidas — cero API keys para el cliente**: Google sigue siendo OAuth
  de 1 clic; Trustpilot, TripAdvisor, Shopify, WooCommerce y TPV se activan pulsando
  «Conectar cuenta» (estado `pending_setup`). La parte técnica queda en `/admin` → pestaña
  «Conexiones», con activación/rechazo cifrados y auditados. Eliminados del panel del cliente
  los campos de token, secreto, API key y la api_key pública de ingesta (ya no viaja al navegador).
  Los enlaces se pegan tal cual (Google Maps o ficha de TripAdvisor) y el sistema extrae el identificador.
- **Header público con glassmorphism y parallax**: barra sticky `z-50`, altura y sombra que se
  contraen al hacer scroll, opacidad del cristal progresiva, blur incrementado y parallax suave
  en el héroe de la landing. Menú móvil accesible y cabecera unificada en páginas legales,
  contacto y sobre-nosotros (`components/SiteHeader.tsx`).
- **UX del panel inspirada en Podium/Birdeye/NiceJob/Trustpilot**: franja de configuración guiada
...
- **Limpieza**: se eliminan los `.md` de auditorías cerradas de la raíz, `proyect.zip` duplicado,
  ...
  selección de `api_key` en las consultas del panel. Documentación y guías sincronizadas con
  el flujo asistido.
- Migración: `supabase/migration_3_14_0.sql` (estado `pending_setup` en `integrations.status`).

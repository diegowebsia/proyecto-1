...
| **Conexiones** ⭐ v3.14.0 | Cola de conexiones asistidas que solicitan los clientes | Completar secretos de webhook/credenciales y activarlas (o rechazarlas) |
...
Desde el panel del cliente ya NO se introduce ninguna clave: pulsa «Conectar cuenta» y la
integración queda en `pending_setup`. En **/admin → Conexiones** ves la cola y por cada
solicitud:

| Proveedor | Qué te pide /admin | De dónde lo sacas |
|---|---|---|
| Shopify / WooCommerce / TPV | Secreto del webhook (HMAC) | App privada de Shopify o `WooCommerce → Settings → Advanced → Webhooks` (URL: `https://tudominio.com/api/integrations/{proveedor}/webhook?tenant={id}`) |
| Trustpilot | API key + Business Unit ID | Trustpilot → Business API → tu cuenta |
| TripAdvisor | Location ID (`dXXXXXX`) | Está en la URL de la ficha del cliente |

Al pulsar **Activar conexión**, las credenciales se cifran (`INTEGRATION_ENCRYPTION_KEY`) y se
registra en `admin_audit_events`. Si no procede, **Rechazar** deja motivo visible en auditoría.
...
| «Mi tienda no envía WhatsApps» | `/admin` → *Conexiones* | Verificar la activación (secreto del webhook), que el pedido esté «Entregado/Completado» **y que haya opt-in** (`skipped: 'no-optin'` en Logs = falta el checkbox del checkout) |
| «No me llegan las opiniones de TripAdvisor» | `/admin` → *Conexiones* | Completar el Location ID (`dXXXXXX`) de la solicitud, revisar `SERPAPI_API_KEY`/`OUTSCRAPER_API_KEY` en el servidor y la cuota `syncs` |
| «No veo dónde pegar mi API key de Trustpilot» | Su panel → *Empresa* → Trustpilot | Ya no hace falta: pulsa «Conectar cuenta» y nosotros lo montamos (v3.14.0) |
| «Llevo horas y la conexión sigue “pendiente”» | `/admin` → *Conexiones* | Alguien de tu equipo debe completar la credencial; revisa la cola a diario |

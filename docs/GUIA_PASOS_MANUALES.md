...
**El cliente (panel `/dashboard` → *Empresa*):** conecta Google en 1 clic (OAuth), pega el
**enlace** de su ficha de Google Maps o de TripAdvisor (el sistema extrae el identificador),
...
queda auditado. La `api_key` por empresa de la API de ingesta se gestiona solo en servidor:
ya no se muestra ni se copia desde el panel del cliente.
...
- [ ] **TripAdvisor**: cuenta SerpAPI u Outscraper → `TRIPADVISOR_PROVIDER` + API key → cada
  empresa pega el **enlace** de su ficha en el panel; tú completas el Location ID en
  `/admin → Conexiones` (v3.14.0) → el cliente pulsa Sincronizar.

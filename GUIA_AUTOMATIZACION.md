...
2. En tu `.env`:
   ...
   # OUTSCRAPER_API_KEY=...
   ```
3. Cada empresa pega el **enlace de su ficha** en el panel → Empresa → TripAdvisor
   (v3.14.0: el backend extrae el código `dXXXXXX` solo; también puedes usar «Conectar
   cuenta» en modo asistido y completar tú el Location ID en `/admin → Conexiones`).

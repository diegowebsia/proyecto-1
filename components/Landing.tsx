...
const faqs = [
  ...
  },
  {
    ...
    a: 'No. La IA prepara borradores (públicos o privados para las quejas) y tú los revisas y publicas en un clic. En tu ficha de Google, la publicación es directa en la plataforma.',
  },
  {
...
export function Landing() {
  /* Parallax suave del héroe: el mockup y los textos suben a distinta
     velocidad al hacer scroll (patrón habitual en SaaS premium). */
  const { scrollY } = useScroll();
  const heroBgY = useTransform(scrollY, [0, 600], [0, 90]);
  const heroCopyY = useTransform(scrollY, [0, 600], [0, 42]);
  const mockupY = useTransform(scrollY, [0, 600], [0, -26]);
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-ink-950 text-ink-100">
      ...
      <section className="relative">
        <motion.div style={{ y: heroBgY }} className="absolute inset-0 -z-0" aria-hidden>
          <Aurora />
        </motion.div>
        <motion.div style={{ y: heroCopyY }} className="relative mx-auto max-w-6xl px-4 pb-20 pt-16 text-center sm:pt-24">
            ...
            <div className="glass overflow-hidden rounded-[1.4rem] text-left shadow-[0_50px_120px_-50px_rgba(2,6,23,1)]">
              ...
              <div className="grid gap-3 p-4 sm:grid-cols-[0.95fr_1.25fr]">
                <div className="space-y-3">
                  {[
                    { n: 'Google Business', l: 'Conectado en 1 clic', d: 'sincronizado hace 2 min', ok: true },
                    { n: 'Trustpilot', l: 'Activado por nuestro equipo', d: 'sincronizado hoy', ok: true },
                  ].map((s) => (
      ...
      <section id="como-funciona" className="relative mx-auto max-w-6xl scroll-mt-24 px-4 py-24">
      ...
      {/* FUNCIONES (bento) */}
      <section id="funciones" className="relative scroll-mt-24 border-y border-white/[0.06] bg-white/[0.012] py-24">

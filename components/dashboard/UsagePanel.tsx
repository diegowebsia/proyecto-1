...
import { Gauge, ShoppingCart, Zap } from 'lucide-react';
...
export function UsagePanel({ tenantId, demo, onBuyAddon, compact = false, previewUsage = null }: UsagePanelProps) {
  const [usage, setUsage] = useState<UsageResponse | null>(previewUsage ?? null);
  const [loading, setLoading] = useState(!demo && !previewUsage);
  const [error, setError] = useState<string | null>(null);
  const [pickerOpen, setPickerOpen] = useState(false);

  const load = useCallback(async () => {
    if (demo || previewUsage) return;
    setLoading(true);
    try {
  ...
  if (demo && !previewUsage) return null;
    ...
    <div className="panel space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-ink-400">
          <Gauge size={13} className="text-brand-300" /> Consumo del ciclo
          {previewUsage && <span className="badge ml-1.5">datos simulados</span>}
          <HelpTip
            title="Consumo del ciclo"
            what="Es el «depósito» de tu plan: peticiones de reseñas, opiniones importadas, respuestas con IA y sincronizaciones. Se recarga solo el día 1 de cada mes."
            helps="Sabrás siempre cuánto te queda: si algo llega al 100 % se pausa con un aviso claro, nunca se cobra de más por sorpresa."
            steps={['Mira los medidores: verde, ámbar o rojo', 'Si uno se agota, amplía con una recarga puntual']}
            helpTopic="cuotas"
            align="end"
          />
        </p>

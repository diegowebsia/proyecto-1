...
export function TriageCard({
  ...
  async function genPrivate() {
    setLoadingMsg(true);
    if (demo) {
      await new Promise((a) => setTimeout(a, 1200));
      setPrivateMsg(
  ...
  async function inspect() {
    setLoadingScan(true);
    if (demo) {
      await new Promise((a) => setTimeout(a, 1300));
      setInspection({
        severity: r.rating <= 2 ? 'alta' : 'media',
        category: 'Trato al cliente',
        legalRisk: false,
        suggestedChannel: 'telefono',
        summary: 'Cliente molest@ por la comunicación de su cita. Reclama explicación; sin amenaza de reclamación legal.',
        actionPlan: ['Llamar hoy mismo', 'Reconocer el fallo sin excusas', 'Ofrecer cita preferente y disculpa personal'],

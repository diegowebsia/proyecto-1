...
export function BillingPanel({
  ...
  const [usage, setUsage] = useState<UsageResponse | null>(previewUsage);
  const [loading, setLoading] = useState(Boolean(tenant) && !demo && !previewUsage);

  useEffect(() => {
    if (!tenant || demo || previewUsage) return;
    let alive = true;
    (async () => {
      setLoading(true);
      try {
    ...
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: EASE }} className="space-y-5">
        ...
        <div className="card relative overflow-hidden">
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="kicker flex items-center gap-2">
                ...
                <HelpTip
                  title="Tu plan"
                  what="Es la tarifa que contrataste: define cuántas gestiones puedes hacer cada mes y qué funciones tienes activadas."
                  helps="Cambiar de plan no te deja colgado: se nota al instante en el mismo panel y solo pagas la diferencia."
                  steps={['Mira el estado arriba: «En prueba» o «Activa»', '¿Te quedas corto? Sube de plan o añade una recarga']}
          ...
          <div className="relative mt-5 grid gap-2.5 sm:grid-cols-3">
            <InfoTile
              ...
              hint={trialActive ? 'ese día empieza el cobro mensual' : 'se renueva automáticamente'}
            />
            <InfoTile
              icon={<Gauge size={14} />}
              label="Peticiones del mes"
              value={usage ? `${usage.metrics.requests.used.toLocaleString('es-ES')} / ${usage.metrics.requests.quota.toLocaleString('es-ES')}` : '—'}
              progress={usage?.metrics.requests.pct}
            />
            <InfoTile
              icon={<Receipt size={14} />}
              label="Almacenamiento"
              value={usage ? `${usage.storage.usedMb} / ${usage.storage.limitMb} MB` : '—'}
              progress={usage?.storage.pct}
            />
          </div>
      ...
      <div className="card">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="flex items-center gap-2 text-base font-bold tracking-tightish text-white">
            Consumo del ciclo actual
            <HelpTip
              title="Consumo y límites"
              what="Cuatro contadores sencillos: peticiones de reseñas, opiniones importadas, respuestas con IA y sincronizaciones automáticas."
              helps="Ninguna función se corta sin avisar: cada medidor pasa de verde a ámbar y a rojo, y verás qué hacer antes de quedarte parado."
              steps={['Si uno llega al 100 %, esa función se pausa', 'Amplía con una recarga puntual (pago único, sin permanencia)']}
              helpTopic="cuotas"
            />
          </h3>
          {usage && (
            <span className="flex items-center gap-1.5">
              {previewUsage && <span className="badge">datos simulados</span>}
...
function InfoTile({
  ...
  label: string;
  value: string;
  /** 0–100 → pinta una minibarra de consumo (comparación visual inmediata). */
  progress?: number;
  hint?: string;
}) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-3 transition-colors duration-200 hover:border-white/[0.14]">
      ...
      {progress !== undefined ? (
        <>
          <span className="meter mt-2" role="presentation">
            <span className={tone} style={{ width: `${pct}%` }} />
          </span>
          <p className="mt-1 text-2xs text-ink-500">{pct}% usado</p>
        </>
      ) : (

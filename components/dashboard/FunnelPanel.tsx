...
export function FunnelPanel({
  ...
  const [stats, setStats] = useState<any>(previewStats);
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="flex items-center gap-2 font-bold text-white">
            ...
            <HelpTip
              title="Embudo de valoración"
              what="Es TU enlace personal para pedir reseñas: el cliente puntúa de 1 a 5★ y le ofrecemos publicar en Google, TripAdvisor o Trustpilot (y un aviso privado si necesita ayuda)."
              helps="Las tiendas con enlace propio multiplican sus reseñas buenas y resuelven quejas antes de que se hagan públicas."
              steps={['Descarga el QR y ponlo en el mostrador', 'Comparte el enlace tras cada venta', 'Mira aquí la conversión cada semana']}
              helpTopic="embudo"
            />
          </h3>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'Votos', value: String(stats.total), icon: <Star size={14} />, hint: 'personas que te han puntuado' },
              { label: 'Nota media', value: stats.avgStars != null ? `${stats.avgStars}★` : '—', icon: <Star size={14} />, hint: '4,5★ o más = excelente' },
              {
                label: 'Conversión a pública',
                value: stats.total ? `${Math.round(((stats.clicks ?? 0) / stats.total) * 100)} %` : '—',
                icon: <TrendingUp size={14} />,
                hint: `${stats.clicks ?? 0} de ${stats.total} fueron a publicar`,
              },
              { label: 'Tickets abiertos', value: String(stats.ticketsOpen), icon: <MessageSquareWarning size={14} />, hint: 'quejas atendidas en privado' },
            ].map((k) => (
              <div key={k.label} className="group rounded-xl border border-white/10 bg-white/[0.02] p-3 transition-all duration-200 hover:border-brand-400/40 hover:bg-brand-500/[0.05]">
                <p className="flex items-center gap-1.5 text-2xs uppercase tracking-wider text-ink-400">
                  {k.icon} {k.label}
                </p>
                <p className="mt-1 text-2xl font-extrabold tabular-nums text-white">{k.value}</p>
                <p className="mt-0.5 text-2xs text-ink-500 opacity-80 transition-opacity group-hover:opacity-100">{k.hint}</p>
              </div>
            ))}
          </div>

...
- [ ] Developers → API keys (live) → `Secret key` (`sk_live_…`) → `STRIPE_SECRET_KEY`.
  La `Publishable key` **ya no se necesita** (v3.14.0): no hay campos de tarjeta en el navegador,
  todo el Checkout se crea en el servidor.

...
export default async function AdminPage() {
  ...
  let pendingConnections: Array<{ id: string; tenant_id: string; tenantName: string; provider: string; created_at: string }> = demo
    ? [
        {
          id: '00000000-0000-4000-8000-0000000000d1',
          tenant_id: demoTenants[1].id,
          tenantName: `${demoTenants[1].name} (ejemplo)`,
          provider: 'shopify',
  ...
  if (admin && guard.ok) {
    ...
    // Cola de conexiones asistidas pendientes (v3.14.0).
    try {
      const { data: pcs } = await admin
        ...
        .limit(50);
      pendingConnections = (pcs ?? []).map((p: any) => ({
        id: p.id,
        tenant_id: p.tenant_id,
        tenantName: p.tenants?.name ?? p.tenant_id,
        provider: p.provider,
        created_at: p.created_at,
      }));
    } catch {
      /* tabla aún sin el estado pending_setup: migración 3.14.0 pendiente */
    }
    } catch {

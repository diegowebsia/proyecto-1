...
type Props = {
  ...
  /** Conexiones asistidas solicitadas por clientes, pendientes de credenciales (v3.14.0). */
  pendingConnections: Array<{ id: string; tenant_id: string; tenantName: string; provider: string; created_at: string }>;
};
...
const TABS = [
  ...
  { id: 'connexions', label: 'Conexiones', icon: Plug },
...
export function AdminClient({ email, demo, tenants, stats, logs, integrations, pendingConnections }: Props) {
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {tab === 'connexions' && (
              <section className="space-y-4">
                <div className="card p-5">
                  <h2 className="flex items-center gap-2 font-bold tracking-tightish text-white">
                    <Plug size={16} className="text-brand-300" /> Conexiones asistidas pendientes
                  </h2>
                  <p className="mt-1 text-sm text-ink-400">
                    Los clientes solo pulsan «Conectar cuenta» en su panel — nunca ven campos
                    técnicos. Aquí completas tú la credencial correspondiente y la conexión pasa a
                    «connected» (se cifra con INTEGRATION_ENCRYPTION_KEY y queda auditada).
                  </p>
                  {pendingConnections.length === 0 ? (
                    <p className="mt-4 flex items-center gap-2 rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] px-3.5 py-3 text-sm text-emerald-100">
                      <CheckCircle2 size={15} /> No hay solicitudes pendientes. Buen trabajo.
                    </p>
                  ) : (
                    <div className="mt-4 space-y-3">
                      {pendingConnections.map((pc) => (
                        <PendingConnectionRow key={pc.id} pc={pc} demo={demo} onDone={() => setTimeout(() => window.location.reload(), 600)} action={action} busy={busy} />
                      ))}
                    </div>
                  )}
                </div>
                  ...
                  <div className="card p-5">
                    ...
                    <ul className="mt-3 space-y-1.5 text-sm text-ink-300">
                      <li className="flex justify-between gap-3">
                        <span>Versión</span>
                        <strong className="font-mono text-2xs text-white">3.14.0</strong>
                      </li>
                      <li className="flex justify-between gap-3">
...
/** Fila de una solicitud de conexión asistida con el formulario técnico (solo admin). */
function PendingConnectionRow({
  ...
  pc: { id: string; tenant_id: string; tenantName: string; provider: string; created_at: string };
  demo: boolean;
  action: (path: string, body: any, key: string, okTitle: string) => Promise<void>;
  busy: string | null;
  onDone: () => void;
}) {
  ...
  const needsWebhook = ['shopify', 'woocommerce', 'store'].includes(pc.provider);
  const isTp = pc.provider === 'trustpilot';
  const key = `pc-${pc.id}`;

  async function complete() {
    const body: Record<string, unknown> = { integrationId: pc.id, action: 'complete', provider: pc.provider };
    if (needsWebhook) body.webhook_secret = secret;
    if (isTp) { body.apiKey = apiKey; body.businessUnitId = unit; }
    if (!needsWebhook && !isTp) body.locationId = loc;
    await action('/api/admin/integrations/complete', body, key, 'Conexión activada');
    onDone();
  }

  async function reject() {
    const reason = window.prompt('Motivo del rechazo (visible en auditoría):')?.trim();
    if (!reason || reason.length < 5) return;
    await action('/api/admin/integrations/complete', { integrationId: pc.id, action: 'reject', provider: pc.provider, reason }, key, 'Solicitud rechazada');
    onDone();
  }
  ...
  return (
    <div className="rounded-2xl border border-amber-400/25 bg-amber-400/[0.05] p-4">
      <p className="flex flex-wrap items-center gap-2 text-sm font-bold text-white">
        ...
        {pc.tenantName}
        <span className="badge-warn">{pc.provider}</span>
        <span className="ml-auto text-2xs font-normal text-ink-400">
          solicitada {new Date(pc.created_at).toLocaleString('es-ES')}
        </span>
      </p>
      {demo ? (
        <p className="mt-2 text-xs text-amber-100/80">
          Dato de ejemplo en modo demo. Conecta Supabase y el formulario estará operativo.
        </p>
      ) : (
        <>
          <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
            {needsWebhook && (
              <input className="input" type="password" placeholder="Secreto del webhook (HMAC)" value={secret} onChange={(e) => setSecret(e.target.value)} aria-label="Secreto del webhook" />
            )}
            {isTp && (
              <div className="grid gap-2 sm:grid-cols-2">
                <input className="input" type="password" placeholder="API key Trustpilot" value={apiKey} onChange={(e) => setApiKey(e.target.value)} aria-label="API key de Trustpilot" />
                <input className="input" placeholder="Business Unit ID" value={unit} onChange={(e) => setUnit(e.target.value)} aria-label="Business Unit ID" />
              </div>
            )}
            {!needsWebhook && !isTp && (
              <input className="input" placeholder="Location ID (dXXXXXX)" value={loc} onChange={(e) => setLoc(e.target.value)} aria-label="Location ID de TripAdvisor" />
            )}
            <button className="btn-primary btn-sm" disabled={!!busy} onClick={complete}>
              {busy === key ? <Spinner label="Guardando…" size={13} /> : <><CheckCircle2 size={13} /> Activar conexión</>}
            </button>
          </div>
          <button className="btn-quiet btn-sm mt-2 text-rose-200" disabled={!!busy} onClick={reject}>
            <XCircle size={13} /> Rechazar solicitud
          </button>
        </>
      )}
    </div>

...
import { OnboardingPresence } from '@/components/dashboard/OnboardingStrip';
...
export function DashboardClient({
  ...
  // Los iconos «?» de cada módulo abren el Centro de ayuda en su sección.
  useEffect(() => {
    function onOpenHelp(e: Event) {
      const detail = (e as CustomEvent<string | null>).detail;
      setHelpTopic(typeof detail === 'string' ? detail : null);
      setHelpOpen(true);
    }
    window.addEventListener('rf:open-help', onOpenHelp);
    return () => window.removeEventListener('rf:open-help', onOpenHelp);
  }, []);
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === 'bandeja' && (
            <motion.div
              ...
              className="space-y-5"
            >
              {/* Configuración guiada (desaparece al completar los pasos) */}
              {primaryTenant && (
                <OnboardingPresence
              ...
              <div id="bandeja-stats" className="scroll-mt-24" />
          ...
          {tab === 'embudo' && (
            ...
            >
              ...
              {tenants.map((t) => (
                <FunnelPanel key={t.id} tenant={t} demo={demo} previewStats={previewStats} />
              ))}
            </motion.div>
          )}
          ...
          {tab === 'facturacion' && (
            <motion.div
              ...
              transition={{ duration: 0.3, ease: EASE }}
            >
              <BillingPanel tenant={primaryTenant} demo={demo} addonResult={addonResult} previewUsage={previewUsage} />
            </motion.div>
          )}
        </AnimatePresence>
      </main>

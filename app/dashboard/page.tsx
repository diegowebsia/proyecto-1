...
export default async function DashboardPage({
  ...
  if (admin && user) {
    try {
      // Nota v3.14.0: `api_key` ya NO se selecciona — las credenciales y claves
      // de ingesta quedan exclusivamente en el servidor (panel /admin).
      const { data: memberships } = await admin
        .from('memberships')
        .select('tenant_id, tenants(id, name, slug, plan, subscription_status, suspended, trial_ends_at, settings)')
        .eq('user_id', user.id);
      ...
      type IntegState = 'connected' | 'pending_setup' | 'error' | 'disconnected';
      const integMap: Record<string, Array<{ provider: string; status: IntegState; last_sync_at: string | null }>> = {};
      if (list.length > 0) {
        ...
        for (const i of integs ?? []) {
          (integMap[i.tenant_id] ??= []).push({
            provider: i.provider,
            status: (i.status ?? 'disconnected') as IntegState,

...
- ✅ **Panel sin fricción técnica (v3.14.0)**: el cliente no ve NINGÚN campo de API keys, tokens ni
  secretos. Google se conecta en 1 clic (OAuth) y Trustpilot/TripAdvisor/Shopify/WooCommerce/TPV con
  **«Conectar cuenta»** (modo asistido `pending_setup`): la parte técnica la completa el dueño desde
  `/admin → Conexiones`, cifrado y auditable. La `api_key` de ingesta ya no viaja al navegador.
...
- ✅ **`/admin` 100 % privado**: el panel interno ya **no se menciona en ninguna página pública**
  (login, registro, landing, ayuda o dashboard). Sigue protegido por `proxy.ts` +
  `lib/authz.ts` y solo entra quien tiene su email en `SUPERADMIN_EMAILS`.
- ✅ **UX clara (benchmark Podium/Birdeye/NiceJob/Trustpilot)**: landing en 3 pasos, comparativa de
  planes, medidores de cuota (verde <70 %, ámbar 70–90 %, rojo >90 %), franja de configuración guiada
  con progreso, KPI de conversión del embudo, tarjetas con minibarras y microinteracciones,
  panel de uso con recargas sugeridas y paywall amable.
...
**Stack:** Next.js 16 (App Router, RSC) · React 19 · TypeScript 5 · Tailwind CSS 3 ·
...
│  proxy.ts (middleware): /admin solo SUPERADMIN · /dashboard y /api/ai|reviews|integrations    │
...
```
POST /api/ai
  → proxy.ts .............. ¿sesión? (401) · ¿suscripción? (402 si no hay plan de pago o caducó el trial)
...
> 💡 Para el cliente: la ayuda de día a día está **dentro del panel** — botón «Ayuda» general y
> un «?» en cada módulo (reseñas, facturación, embudo, cuota, tienda, conexiones) que abre la
> sección concreta en lenguaje no técnico. Las guías extensas viven en `docs/`, fuera de la vista
> del cliente.
...
Registro → /bienvenido (2 planes de pago)
  ...
  → día 8 sin pago: el proxy corta /dashboard → /bienvenido?reason=trial-ended
```
...
- **Componentes**: `Motion`, `Accordion`, `Toast`, `Skeleton`, `Quota` (medidores + topes de BD),
  `AddonPicker`, `HelpCenter` (temas por módulo), `HelpTip` («?» accesible), `SiteHeader`
  (glass + parallax), `OnboardingStrip` (configuración guiada), `CookieBanner`.
...
│       ├── tenants/    start · usage · settings · route        · admin/ (impersonate·tenants·integrations/complete)
│       ├── demo/       access (canje del código de pruebas por cookie firmada)
│       └── health · contact · auth/signout
├── components/          SiteHeader (glass+parallax) · Landing · Footer · LegalLayout · AuthForm ·
│   │                    Wizard · Accordion · Motion · Toast · Skeleton · Quota · AddonPicker ·
│   │                    HelpCenter (temas por módulo) · HelpTip («?» contextual) · CookieBanner
│   └── dashboard/       dashboard-client · OnboardingStrip · BillingPanel · TenantCard · StoreConnect ·
...
├── proxy.ts             corte de acceso por plan/suscripción y rol (middleware de Next 16)
...
- `/admin` y `/api/admin/*`: **doble verificación** (proxy + guard de servidor) y **sin
